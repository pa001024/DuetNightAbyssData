...
function M:InitWidget(MonsterEid, Name)
  self.MonsterEid = MonsterEid
  self.LastHp = nil
  self.LastShield = nil
  self.LastHpNumber = nil
  self.LastShieldNumber = nil
  ...
  local NumBloodNormalColor = FSlateColor()
  NumBloodNormalColor.SpecifiedColor = FLinearColor(0.258183, 0.590619, 0.318547, 1.0)
  rawset(self, "NumBloodNormalColor", NumBloodNormalColor)
  local NumBloodCriticalColor = FSlateColor()
  NumBloodCriticalColor.SpecifiedColor = FLinearColor(1.0, 0.048, 0.048, 1.0)
  rawset(self, "NumBloodCriticalColor", NumBloodCriticalColor)
  ...
  if self.DefenseCoreName and Name then
    self.DefenseCoreName:SetText(GText(Name))
  end
...
function M:GetTargetActor()
  ...
  if not self.MonsterEid or self.MonsterEid <= 0 then
    return nil
  end
  local Target = Battle(self):GetEntity(self.MonsterEid)
...
function M:UpdateTargetInfo()
  ...
  local Num_Shield = self.Num_Shield
  local Num_Blood = self.Num_Blood
  local ShieldBar = self.ShieldBar
  local HpBar = self.HpBar
  local LastShield = self.LastShield
  local LastHp = self.LastHp
  local LastHpNumber = self.LastHpNumber
  local LastShieldNumber = self.LastShieldNumber
  if ShieldBar and LastShield ~= NewShield then
    local NewShieldPercent = NewShield / MaxShieldSafe
    if nil == LastShield then
  ...
  if HpBar and LastHp ~= NewHp then
    HpBar:SetBarPercent(NewHpPercent)
    if nil ~= LastHp and NewHp < LastHp then
      HpBar:PlayDeduct(true)
    end
  end
  local NewHpNumber = math.floor(NewHp)
  if (not LastHpNumber or LastHpNumber ~= NewHpNumber) and Num_Blood then
    Num_Blood:SetText(NewHpNumber)
  end
  local NewShieldNumber = math.floor(NewShield)
  if (not LastShieldNumber or LastShieldNumber ~= NewShieldNumber) and Num_Shield then
    Num_Shield:SetText(NewShieldNumber)
  end
  local IsLowHp = NewHpPercent < self.LowHpPercent
  if self.IsLowHp ~= IsLowHp then
    self.IsLowHp = IsLowHp
    if Num_Blood then
      local NumColor = IsLowHp and self.NumBloodCriticalColor or self.NumBloodNormalColor
      Num_Blood:SetColorAndOpacity(NumColor)
    end
    if HpBar then
      local BarColor = IsLowHp and self.CriticalColor or self.NormalColor
      HpBar:SetProgressBarColorInner(BarColor, nil, "ProgressBar")
    end
  end
  self.LastShield = NewShield
  self.LastHp = NewHp
  self.LastHpPercent = NewHpPercent
  self.LastHpNumber = NewHpNumber
  self.LastShieldNumber = NewShieldNumber
end
