...
function M:Init()
  if self.Text then
    self.Text:SetText(GText("Minigame_Textmap_100304"))
  end
...
function M:BindReddot()
  ...
  ReddotManager.AddListener("DetectiveQuestion", self, self.RefreshRedDot)
...
function M:RefreshRedDot()
  ...
  if IsNewClue then
    if self.Bubble and self.Bubble.ShowInfo then
      self.Bubble:ShowInfo()
    end
    if self.Panel_Clue then
      self.Panel_Clue:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    end
    if self.New then
  ...
  elseif IsNewQuestion then
    if self.Panel_Clue then
      self.Panel_Clue:SetVisibility(UIConst.VisibilityOp.Collapsed)
    end
    if self.New then
