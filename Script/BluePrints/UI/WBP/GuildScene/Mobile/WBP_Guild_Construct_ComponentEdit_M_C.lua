...
function M:Construct()
  ...
  self:BindToAnimationFinished(self.Parameter_In, {
    self,
    function()
  ...
  self:BindToAnimationFinished(self.Parameter_Out, function()
...
function M:UpdateRotation(DeltaRotation)
  ...
  local TextNum = GText(tostring(ActorState.LocalRotation.Yaw) .. "°")
  self.Parameter_Rotation.Text_Parameter:SetText(TextNum)
  self:HandleAnimation()
  self:UpdatePanelParameter(self.Parameter_Rotation.Text_Name:GetText(), TextNum)
end
...
function M:UpdateScale(DeltaScale)
  ...
  local TextNum = GText(string.format("%.1f", ActorState.LocalScale))
  self.Parameter_Scale.Text_Parameter:SetText(TextNum)
  self:HandleAnimation()
  self:UpdatePanelParameter(self.Parameter_Scale.Text_Name:GetText(), TextNum)
end
...
function M:UpdateHeight(DeltaHeight)
  ...
  local TextNum = GText(tostring(ActorState.LocalHeight))
  self.Parameter_Height.Text_Parameter:SetText(TextNum)
  self:HandleAnimation()
  self:UpdatePanelParameter(self.Parameter_Height.Text_Name:GetText(), TextNum)
end
...
function M:HandleAnimation()
  if not self:IsAnyAnimationPlaying() and not self.bAnimationPlaying then
    self:PlayAnimation(self.Parameter_In)
    self.bAnimationPlaying = true
  end
end

function M:UpdatePanelParameter(TextType, TextNum)
  if not TextType or not TextNum then
    return
  end
  self.Text_Type:SetText(TextType)
  self.Text_Num:SetText(TextNum)
end
