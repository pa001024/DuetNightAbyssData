...
function M:Construct()
  ...
  if self.Tag_Room and self.Tag_Room.TextTag then
    self.Tag_Room.TextTag:SetText(GText("UI_AsyncCombat_Host"))
  end
  if self.Tag_Mvp and self.Tag_Mvp.TextTag then
...
function M:OnFocusReceived(MyGeometry, InFocusEvent)
  if self.Content and not self.Content.Empty and self.ParentWidget then
    self.ParentWidget:OnListRankItemClicked(self.Content)
  end
...
function M:InitPlayerInfo()
  ...
  local IsEmptyFrame = self.RankInfo.TitleBefore ~= nil and nil ~= self.RankInfo.TitleAfter and self.RankInfo.TitleBefore < 0 and self.RankInfo.TitleAfter < 0
...
function M:InitPlayerTag()
  if not self.RankInfo then
    return
  end
  local IsMaster = self.RankInfo.IsMaster or false
  local IsMvp = self.RankInfo.IsMvp or false
  if IsMaster then
    if self.WS_Tag then
      self.WS_Tag:SetVisibility(UIConst.VisibilityOp.Visible)
      self.WS_Tag:SetActiveWidgetIndex(0)
    else
      if self.Tag_Room then
        self.Tag_Room:SetVisibility(UIConst.VisibilityOp.Visible)
      end
      if self.Tag_Mvp then
        self.Tag_Mvp:SetVisibility(UIConst.VisibilityOp.Collapsed)
      end
    end
  elseif IsMvp then
    if self.WS_Tag then
      ...
      if self.Tag_Room then
        self.Tag_Room:SetVisibility(UIConst.VisibilityOp.Collapsed)
      end
      if self.Tag_Mvp then
        self.Tag_Mvp:SetVisibility(UIConst.VisibilityOp.Visible)
      end
    end
  elseif self.WS_Tag then
...
function M:HeadMenuOpenChanged(bOpen)
  ...
  self:SetFocus()
  if self.ParentWidget.OnListRankItemClicked then
    self.ParentWidget:OnListRankItemClicked(self.Content)
  end
end
