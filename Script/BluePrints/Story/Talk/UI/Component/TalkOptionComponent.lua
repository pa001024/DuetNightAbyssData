...
function M:ListenUpDownEvents()
  self.Owner:ListenForInputAction("TalkUpSelect", EInputEvent.IE_Pressed, true, {
    self.Owner,
    function()
      self:UpSelectAction()
    end
  })
  self.Owner:ListenForInputAction("TalkDownSelect", EInputEvent.IE_Pressed, true, {
    self.Owner,
    function()
...
function M:UnlistenUpDownEvents()
  self.Owner:StopListeningForInputAction("TalkUpSelect", EInputEvent.IE_Pressed)
  self.Owner:StopListeningForInputAction("TalkDownSelect", EInputEvent.IE_Pressed)
end
...
function M:SwitchEnableConfirmEvents(bEnable)
  if bEnable then
    ...
    self.Owner:ListenForInputAction("TalkOption", EInputEvent.IE_Pressed, true, {
      self.Owner,
      function()
        self:OnCurrentItemPressed()
      end
    })
    self.Owner:ListenForInputAction("TalkOption", EInputEvent.IE_Released, true, {
      self.Owner,
      function()
    ...
    self.bListeningConfirmEvents = false
    self.Owner:StopListeningForInputAction("TalkOption", EInputEvent.IE_Pressed)
    self.Owner:StopListeningForInputAction("TalkOption", EInputEvent.IE_Released)
  end
end
...
function M:SwitchUpDownSelectAction(bUp)
  local CurrentTime = UE4.UGameplayStatics.GetRealTimeSeconds(self.Owner)
