local T = {}
T.RT_1 = {
  99,
  1004,
  1003
}
T.RT_2 = {1004, 1003}
local LocalTimeProxy = (DataMgr or {}).LocalTimeProxy or function(x)
  return x
end
local ReadOnly = (DataMgr or {}).ReadOnly or function(n, x)
  return x
end
return ReadOnly("SkinGacha", {
  [9001] = {
    ConditionId = 4041,
    DisplayCostRes = {
      99,
      100,
      1001
    },
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = {1001},
    GachaDes = "SkinGacha_Des_Normal",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_SongluShuimu01.Gacha_Pool_SongluShuimu01'",
    GachaDisplayType = 1,
    GachaHistoryType = 2,
    GachaId = 9001,
    GachaName = "SkinGacha_Name_Normal_1",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Normal.WBP_Gacha_PoolTitle_Normal'",
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_NORMAL",
    GachaWarning = "SkinGacha_Warning_Normal",
    IsHIdeCountdown = true,
    ProbabilityId = 9999,
    Sequence = 1,
    Star3ItemId = 99993,
    Star4ItemId = 99992,
    Star5ItemId = 99991,
    TabId = 1
  },
  [9002] = {
    BgBGM = "event:/bgm/1_0/0088_saiqi_skin",
    BgMusic = "event:/ui/btc/gacha_pool_saiqi_skin01",
    BubbleShowGoods = {
      140335,
      140336,
      140337
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Saiqi01.Gacha_Pool_Saiqi01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1785142800),
    GachaHistoryType = 2,
    GachaId = 9002,
    GachaName = "SkinGacha_Name_Special_1",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Saiqi01.WBP_Gacha_PoolTitle_Saiqi01'",
    GachaStartTime = LocalTimeProxy(1780279200),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL",
    GachaWarning = "SkinGacha_Warning_Special",
    ProbabilityId = 1001,
    Sequence = 2,
    Star3ItemId = 99983,
    Star4ItemId = 99982,
    Star5ItemId = 99981,
    TabId = 2
  },
  [9003] = {
    BgBGM = "event:/bgm/1_1/0133_fushu_skin",
    BgMusic = "event:/ui/btc/gacha_pool_fushu_skin01",
    BubbleShowGoods = {
      140421,
      140422,
      140423
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Baiheng01.Gacha_Pool_Baiheng01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1788771600),
    GachaHistoryType = 2,
    GachaId = 9003,
    GachaName = "SkinGacha_Name_Special_2",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Baiheng01.WBP_Gacha_PoolTitle_Baiheng01'",
    GachaStartTime = LocalTimeProxy(1785117600),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_2",
    GachaWarning = "SkinGacha_Warning_Special_2",
    ProbabilityId = 1001,
    Sequence = 3,
    Star3ItemId = 99973,
    Star4ItemId = 99972,
    Star5ItemId = 99971,
    TabId = 3
  },
  [9004] = {
    BgBGM = "event:/bgm/1_2/0136_zhiliu_skin01",
    BgMusic = "event:/ui/btc/gacha_pool_zhiliu_skin01",
    BubbleShowGoods = {
      140496,
      140497,
      140498,
      140499
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Zhiliu01.Gacha_Pool_Zhiliu01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1792400400),
    GachaHistoryType = 2,
    GachaId = 9004,
    GachaName = "SkinGacha_Name_Special_3",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Zhiliu01.WBP_Gacha_PoolTitle_Zhiliu01'",
    GachaStartTime = LocalTimeProxy(1788746400),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_3",
    GachaWarning = "SkinGacha_Warning_Special_3",
    ProbabilityId = 1001,
    Sequence = 4,
    Star3ItemId = 99963,
    Star4ItemId = 99962,
    Star5ItemId = 99961,
    TabId = 4
  },
  [9005] = {
    BgBGM = "event:/bgm/1_3/0159_suyi_skin01",
    BgMusic = "event:/ui/btc/gacha_pool_suyi_skin01",
    BubbleShowGoods = {
      140294,
      140295,
      140296
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Suyi01.Gacha_Pool_Suyi01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1780304400),
    GachaHistoryType = 2,
    GachaId = 9005,
    GachaName = "SkinGacha_Name_Special_4",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Suyi01.WBP_Gacha_PoolTitle_Suyi01'",
    GachaStartTime = LocalTimeProxy(1775440800),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_4",
    GachaWarning = "SkinGacha_Warning_Special_4",
    ProbabilityId = 1001,
    Sequence = 5,
    Star3ItemId = 99953,
    Star4ItemId = 99952,
    Star5ItemId = 99951,
    TabId = 5
  },
  [9007] = {
    BgBGM = "event:/bgm/1_5/0175_eve_skin01",
    BgMusic = "event:/ui/btc/gacha_pool_yiwei_skin01",
    BubbleShowGoods = {
      140389,
      140390,
      140391
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Eve01.Gacha_Pool_Eve01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1788771600),
    GachaHistoryType = 2,
    GachaId = 9007,
    GachaName = "SkinGacha_Name_Special_6",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Eve01.WBP_Gacha_PoolTitle_Eve01'",
    GachaStartTime = LocalTimeProxy(1785117600),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_6",
    GachaWarning = "SkinGacha_Warning_Special_6",
    ProbabilityId = 1001,
    Sequence = 7,
    Star3ItemId = 99933,
    Star4ItemId = 99932,
    Star5ItemId = 99931,
    TabId = 7
  },
  [9008] = {
    BgMusic = "event:/ui/btc/gacha_pool_falu_skin01",
    BubbleShowGoods = {
      140462,
      140456,
      140457,
      140458
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Falu01.Gacha_Pool_Falu01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1792400400),
    GachaHistoryType = 2,
    GachaId = 9008,
    GachaName = "SkinGacha_Name_Special_7",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Falu01.WBP_Gacha_PoolTitle_Falu01'",
    GachaStartTime = LocalTimeProxy(1788746400),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_7",
    GachaWarning = "SkinGacha_Warning_Special_7",
    ProbabilityId = 1001,
    Sequence = 8,
    Star3ItemId = 99923,
    Star4ItemId = 99922,
    Star5ItemId = 99921,
    TabId = 8
  },
  [90021] = {
    BgBGM = "event:/bgm/1_4/0174_skin_fuluo01",
    BgMusic = "event:/ui/btc/gacha_pool_fuluo_skin01",
    BubbleShowGoods = {
      140356,
      140357,
      140358
    },
    ConditionId = 4041,
    DisplayCostRes = T.RT_1,
    GachaCoreDes = "UI_SkinGacha_Core_GoldSkin",
    GachaCoreDesColor = 5,
    GachaCostNum10 = 10,
    GachaCostRes = T.RT_2,
    GachaDes = "SkinGacha_Des_Special",
    GachaDisplayPath = "FileMediaSource'/Game/Asset/UIVideo/Gacha/Gacha_Pool_Fuluo01.Gacha_Pool_Fuluo01'",
    GachaDisplayType = 1,
    GachaEndTime = LocalTimeProxy(1785142800),
    GachaHistoryType = 2,
    GachaId = 90021,
    GachaName = "SkinGacha_Name_Special_5",
    GachaNameBp = "WidgetBlueprint'/Game/UI/WBP/GachaNew/Widget/Title/WBP_Gacha_PoolTitle_Fuluo01.WBP_Gacha_PoolTitle_Fuluo01'",
    GachaStartTime = LocalTimeProxy(1780279200),
    GachaTimes = 1,
    GachaType = "GACHA_SKIN_TYPE_SPECIAL_5",
    GachaWarning = "SkinGacha_Warning_Special_5",
    ProbabilityId = 1001,
    Sequence = 6,
    Star3ItemId = 99943,
    Star4ItemId = 99942,
    Star5ItemId = 99941,
    TabId = 6
  }
})
