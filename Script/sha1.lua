...
function asHEX(a)
  ...
  local i = 1
  while i < #a do
    local binary = (a[i + 3] and "1" or "0") .. (a[i + 2] and "1" or "0") .. (a[i + 1] and "1" or "0") .. (a[i + 0] and "1" or "0")
...
function sha1(msg)
  ...
  local first_append = string.char(128)
  local non_zero_message_bytes = #msg + 1 + 8
  local current_mod = non_zero_message_bytes % 64
  local second_append = ""
  if 0 ~= current_mod then
    second_append = string.rep(string.char(0), 64 - current_mod)
  end
  local B1, R1 = math.modf(msg_len_in_bits / 16777216)
  local B2, R2 = math.modf(16777216 * R1 / 65536)
  local B3, R3 = math.modf(65536 * R2 / 256)
  local B4 = 256 * R3
  local L64 = string.char(0) .. string.char(0) .. string.char(0) .. string.char(0) .. string.char(B1) .. string.char(B2) .. string.char(B3) .. string.char(B4)
...
function hmac_sha1(key, text)
  ...
  local key_xord_with_0x36 = key:gsub(".", xor_with_0x36) .. string.rep(string.char(54), blocksize - #key)
  local key_xord_with_0x5c = key:gsub(".", xor_with_0x5c) .. string.rep(string.char(92), blocksize - #key)
