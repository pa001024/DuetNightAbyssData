...
function M:ReceiveStartTest()
  ...
  local QuestChainProxy = require("BluePrints.Client.Quest.QuestChainProxy")
  local TestQuestChainId = 123456
  local MockStoryPath = "Test/QuestChainProxyTest123456.story"
  ...
  local OriginalGetQuestChain = QuestChainProxy.GetQuestChain
  
  function QuestChainProxy.GetQuestChain(CQChain)
    return MockQuestChainData[CQChain.QuestChainId]
  end
  
  local function Restore()
    QuestChainProxy.GetQuestChain = OriginalGetQuestChain
  end
  
  local Chain = QuestChainProxy(TestQuestChainId)
  if not AssertTrue(nil ~= Chain, "QuestChainProxy should not be nil after creation") then
    Restore()
    return
  end
  ...
  if Chain:IsStorylineRunning() then
    ...
    if not AssertEqual(true, DoingNode.HasFinished, "FailDoingQuest should trigger FailQuest on the doing StoryNode") then
      Restore()
      return
    end
    Chain:StopStoryline()
  ...
  Restore()
  self:FinishTest(UE4.EFunctionalTestResult.Succeeded, "QuestChainProxyTests: All tests passed")
end
